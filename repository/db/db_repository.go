...
func (r *dbRepository) GetById(id string) (*access_token.AccessToken, *errors.RestError) {
	...
	if err := cassandra.GetSession().Query(getAccessToken, id).Scan(&result.AccessToken, &result.UserId, &result.ClientId, &result.Expires); err != nil {
...
func (*dbRepository) Create(at access_token.AccessToken) *errors.RestError {
	...
	if err := cassandra.GetSession().Query(CreateAccessToken, at.AccessToken, at.UserId, at.ClientId, at.Expires).Exec(); err != nil {
...
func (*dbRepository) UpdateExpirationTime(at access_token.AccessToken) *errors.RestError {
	...
	if err := cassandra.GetSession().Query(UpdateExpires, at.Expires, at.AccessToken).Exec(); err != nil {
		return errors.NewInternalServerError(err.Error())
	}
